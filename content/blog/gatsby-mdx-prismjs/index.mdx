...
<CodeNote href="https://github.com/rutterjt/blog-examples/tree/main/gatsby-mdx-prismjs" />

## The Goal

Code blocks are written in MDX by enclosing text in triple backticks (with the opening set optionally followed by the name of the coding language). The goal for this project will be to convert native MDX codeblocks like this into syntax-highlighted code. So, for example, we would write this in our MDX code:
...
And it would render like this on our blog:
...
`<Highlight />` accepts a number of props:

- `code`, which is the text that we want to apply syntax highlight to. Here, it's stored in `exampleCode`, and directly passed to `<Highlight />`.
...
- `defaultProps`, which is exported from the package and is simply destructured as props directly into the component.
- A [render prop](https://reactjs.org/docs/render-props.html), passed as `children`. This is where the main work of rendering the code block is done. `<Highlight />` works by parsing the input code line-by-line, separating each line into 'tokens', which represent specific bits of code (e.g., variables, strings, functions). `<Highlight />` then exposes all the line/token data to our render function, and we can then map over it to render the code. For our purposes, we'll just copy and paste the render function from the example. But if you want to modify how the code gets displayed (e.g., adding line numbers or individual line diff highlighting), this is where you'd do it.

As you can see, this is a lot of code to write every time we want to render a code block. Ideally, we'd like to have one component that accepts the code as a prop, and then passes it directly to `<Highlight />`. Let's create that component now.

## Creating a `<CodeBlock />` Component

Here's the starting point for our component. Most of this is just copied from the `prism-react-renderer` docs:
...
If you use TypeScript, you may have noticed that `code` is type `any` by default, which is [usually a no-no](https://www.typescriptlang.org/docs/handbook/declaration-files/do-s-and-don-ts.html#any). This probably isn't an issue though, because the way MDX works should ensure that whatever ends up getting parsed as `children.props.children` will be a string. But if you're concerned, you can feel free to add some type checks, to be safe.
...
As you can see, whatever language is specified after the triple backticks gets added to the `<code />` element's `className`, as `class="language-<language>"`. So to parse the language, we'll need to get fancy with some regular expressions.
...
Putting it all together, our shiny new `<CodeBlock />` component looks like:
...
There are a lot of ways that you can customize your new syntax-highlighted code blocks. Most of them (like adding line-numbers or diff highlighting) involve getting into the nitty-gritty of `<Highlight />`'s render function. Those really deserve their own post, and I won't get into them here. But we can look at one really easy way of customizing your code blocks: themes.
...
But you're not just limited to the built-in themes. You can also add a custom Prism CSS file for your theme. There a bunch of great [Prism themes](https://github.com/PrismJS/prism-themes) available, so take a look if you're interested!
